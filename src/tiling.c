...
void wTileRelayout(WTileStrip *strip, WArea usableArea)
{
	...
	for (col = strip->first; col != NULL; col = col->next) {
		...
		if (col->window && col->window->flags.mapped) {
			...
			/* Schneidet das Fenster ab, sobald es den Dock-Bereich (usableArea.x1) berührt */
			if (win_x < usableArea.x1) {
				int overlap = usableArea.x1 - win_x;
				win_x = usableArea.x1;     /* Linke Kante am Dock anheften */
				win_w = win_w - overlap;   /* Breite um den überlappenden Teil kürzen */
			}

			/* Nur zeichnen, wenn noch ein sichtbarer Teil übrig ist */
			if (win_w > 0) {
				wWindowConfigure(col->window, win_x, usableArea.y1,
						 win_w, usableArea.y2 - usableArea.y1);
			} else {
				/* Komplett aus dem sichtbaren Bereich geschoben */
