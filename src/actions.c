...
void wSetFocusTo(WScreen *scr, WWindow *wwin)
{
	...
	if (wwin->flags.mapped && !WFLAGP(wwin, no_focusable)) {
		WTileStrip *strip = scr->workspaces[wwin->frame->workspace]->tile_strip;

		if (strip) {
			WArea usableArea = wGetUsableAreaForHead(scr,
				wGetHeadForPointerLocation(scr), NULL, True);

			wTileScrollToWindow(strip, wwin, usableArea);
		}
...
void wMaximizeWindow(WWindow *wwin, int directions, int head)
{
...

	{
		...
		if (strip)
			wTileSetLocked(strip, wwin, True);
	}
...
void wUnmaximizeWindow(WWindow *wwin)
{
	...
	{
		WTileStrip *strip = wwin->screen_ptr->workspaces[wwin->frame->workspace]->tile_strip;

		if (strip) {
			WArea usableArea = wGetUsableAreaForHead(wwin->screen_ptr,
				wGetHeadForPointerLocation(wwin->screen_ptr), NULL, True);

			wTileSetLocked(strip, wwin, False);
