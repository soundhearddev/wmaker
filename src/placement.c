...
static void
smartTileWindow(WWindow *wwin, int *x_ret, int *y_ret, unsigned int *width,
                 unsigned int *height, WArea usableArea)
{
	WScreen *scr = wwin->screen_ptr;
	WTileStrip *strip;

	if (WFLAGP(wwin, no_tiling)) {
		if (!center_place_window(wwin, x_ret, y_ret, *width, *height, usableArea)) {
			/* Fenster zu groß für zentrierte Platzierung -> obere linke Ecke,
			 * Originalgröße unangetastet lassen */
			*x_ret = usableArea.x1;
			*y_ret = usableArea.y1;
		}
		return;
	...
	strip = scr->workspaces[scr->current_workspace]->tile_strip;
	wTileAddWindow(strip, wwin, usableArea, x_ret, y_ret, width, height);
}
